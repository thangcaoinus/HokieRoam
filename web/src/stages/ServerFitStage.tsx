...
export default function ServerFitStage() {
  ...
  // Shared by both branches: with a placement it sits under the verdict, without one it is the
  // only thing to do on the sheet.
  const Actions = () => <>
  ...
  return <div className="stage">
    ...
    {p ? <>
      ...
      {/* The measurement is the product, so it leads the sheet rather than hiding in a
          disclosure, and it is read BEFORE the actions it should inform. */}
      <div className="measure" data-testid="placement-status" data-verdict={p.plan_fit} role="status">
        <div className={`measure-cell verdict ${p.plan_fit}`}>
          <div className="v">{p.plan_fit}</div>
          <div className="k">Placement</div>
        </div>
        {([
          ['Footprint IoU', `${(p.selected.metrics.iou * 100).toFixed(1)}`, '%'],
          ['Coverage', `${(p.selected.metrics.coverage * 100).toFixed(1)}`, '%'],
          ['Spill', p.selected.metrics.spill_area_m2.toFixed(1), 'm²'],
          ['Neighbour overlap', p.selected.metrics.neighbor_overlap_m2.toFixed(2), 'm²'],
        ] as const).map(([k, v, unit]) => (
          <div className="measure-cell" key={k}>
      ...
      <p className="measure-note">
        {p.plan_fit === 'rejected' ? 'Outside the fit constraints — explore for inspection, not as an approved placement.' : p.plan_fit === 'review' ? 'Inspect the fit before using this result.' : 'Plan fit passed.'}
        {' '}Heading unverified · {p.height === 'inferred' ? 'height inferred from the mesh' : `height ${p.request.measured_height_m?.toFixed(1)} m from the ${p.provenance.source.toUpperCase()} record`}.
      </p>
      <Actions />

      <div className="card" style={{ padding: 6, marginTop: 18 }}>
        <div className="row wrap" style={{ padding: 10 }}>
          ...
          {/* A drawn key: a colour named in prose drifts the moment the palette moves. */}
          {raw ? <span className="dimmer">Unchanged source coordinates</span> : <span className="legend">
            <span><i style={{ background: 'var(--vermilion)' }} />Authoritative footprint</span>
            <span><i style={{ background: 'var(--ink)' }} />Conservative mesh hull</span>
          </span>}
        </div>
      ...
      <details className="card card-pad" style={{ marginTop: 18 }}>
        <summary>Inspect the transform</summary>
        <p style={{ marginTop: 14 }}>Plan scale: {p.selected.scale.toFixed(3)}{p.selected.scale_y ? ` · Vertical scale: ${p.selected.scale_y.toFixed(3)} (from the recorded height, not the plan fit)` : ' · Vertical scale: uniform'} · Yaw: {(p.selected.yaw_radians * 180 / Math.PI).toFixed(1)}° · Neighbors: {p.neighbor_check}</p>
