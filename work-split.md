...
decision down, and make the other engine defer or go away.** Recommendation: the Python one is stricter
and produces the manifest, so make it authoritative and let the browser keep a preview-only path.
...
## P6 — Sponsor / world registration (blocked, not schedulable)

Needs the Procedura interface contract before anything can be written. Until someone has it, this lane
does not exist. Per `implementation-plan.md` §6: if it is still blocked at hour 10, ship the manifest and
record exactly what remains unverified. A receipt plus a reload is integration; anything else is not, and
